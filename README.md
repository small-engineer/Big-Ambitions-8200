...
通常求人へ約1%で著名ハッカーが現れる。能力値が高い代わりに、給与要求、設備要求、勤務条件も厳しい。

| 候補 | 固有特性 | ゲーム効果 |
|---|---|---|
| Kevin Mitnick | Social Engineer | 人的情報源と交渉に強い |
| George Hotz | Reverse Engineer | 技術分析が速いが単独作業を好む |
| Joanna Rutkowska | System Isolation | 防御設計と証拠保全に強い |
| Mudge | Security Engineering | チーム全体の検知・防御能力を上げる |
| Tsutomu Shimomura | Incident Hunter | 帰属分析とインシデント対応に強い |

同名候補は1セーブにつき1人だけ。通常のHackerを不要にする万能キャラにはせず、専門分野へ強い尖った人材にする。
...
競合会社は売上を奪うだけでなく、OSINT会社の情報品質、人材、信用を狙う。プレイヤー側も週1回、競合へオペレーションを仕掛けられる。

| 競合の妨害 | 起きること | 主な防御 |
|---|---|---|
| 証拠汚染 | 誤ったFindingが混ざり、納品品質が落ちる | 出典履歴、独立ソースでの裏取り |
| 評判工作 | 市場需要と契約単価が一時低下する | 透明な訂正、証拠付きレポート |
| 人材引き抜き | Hackerへ競合から高額オファーが届く | 給与、研修、勤務環境、チーム満足度 |
| インフラ偵察 | サーバー障害イベントの確率が上がる | アクセス分離、監視、復旧テスト |

プレイヤーが選べるオペレーションは、`囮情報を置く`、`競合レポートを検証する`、`人材を引き抜く`、`価格攻勢をかける`。成功率はHackerの専門性と証拠の確度で決まり、根拠の弱い攻勢は逆に信用を失う。

画面上の主要値は4つに絞る。

```text
Integrity    情報品質。低いと誤報が増える
Resilience   妨害と障害への耐性
Attribution  誰の工作かを見極める確度
Heat         強引な工作で高まり、契約喪失や反撃を招く
```

これにより、単純な攻撃力競争ではなく、出典管理、裏取り、検知、復旧、帰属の不確実性を経営判断として学べる。
...
- 週1回の競合イベント
- 証拠汚染、評判工作、人材引き抜き、インフラ偵察
- Integrity / Resilience / Attribution / Heat
- Legendary Candidateと固有特性
- 防御投資と反撃の選択

完了条件: 裏取り・監視・復旧へ投資した会社が妨害後の損失を抑え、根拠の弱い反撃は自社へ損害が返る。
